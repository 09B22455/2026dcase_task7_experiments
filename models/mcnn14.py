...
class MCnn14(nn.Module):
    ...
    def extract_feature(self, input, task=1):
        ...
        x = self.conv_block1(x,(2,2),'avg',task)
        x = F.dropout(x,p=0.2,training=self.training)

        x = self.conv_block2(x,(2,2),'avg',task)
        x = F.dropout(x,p=0.2,training=self.training)

        x = self.conv_block3(x,(2,2),'avg',task)
        x = F.dropout(x,p=0.2,training=self.training)

        x = self.conv_block4(x,(2,2),'avg',task)
        x = F.dropout(x,p=0.2,training=self.training)

        x = self.conv_block5(x,(2,2),'avg',task)
        x = F.dropout(x,p=0.2,training=self.training)

        x = self.conv_block6(x,(2,2),'avg',task)
        x = F.dropout(x,p=0.2,training=self.training)
